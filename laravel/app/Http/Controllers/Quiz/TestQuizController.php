...
class TestQuizController extends Component
{
    public $quote;
    public $quizid;
    public $banksoals;
    ...
    {
        $this->banksoals = BankSoal::withcount('questions')->where('is_active', '1')
            ->orderBy('id')
            ->get();
    ...
    {
        //Return a random question from the banksoal selected by the user for quiz.
        // disabled because having issues with shuffle, it works but in a wierd way.

        // $question = Question::where('banksoal_id', $this->banksoalId)
        ...
        $question = Question::where('bank_soal_id', $this->banksoalId)
            ->whereNotIn('id', $this->answeredQuestions)
            ->with('answers')
            ->inRandomOrder()
            ->first();

        //If the quiz size is greater then actual questions available in the quiz banksoals,
        //Finish the quiz and take the user to results page on exhausting all question from a given banksoal.
        if ($question === null) {
    ...
    public function nextQuestion()
    {
        ...
        Quiz::create([
            ...
            'bank_soal_id' => $this->currentQuestion->banksoal_id,
